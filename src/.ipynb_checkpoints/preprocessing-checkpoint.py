...
# COMPATIBILITY WRAPPERS (untuk train_rl.py)

def tokenize_simple(text: str) -> List[str]:
    """
    Wrapper untuk tokenize dengan SPECIAL_TOKENS sebagai default.
    Dipanggil oleh train_rl.py dengan 1 argumen.
    """
    return tokenize(text, SPECIAL_TOKENS)


def tokens_to_indices(tokens: List[str], word2idx: Dict[str, int]) -> List[int]:
    """
    Wrapper untuk train_rl.py dan evaluate.py.
    Mengonversi token → indeks dalam vocabulary BIASA (bukan extended).
    Token OOV (tidak ada di vocab) dipetakan ke <unk> agar aman untuk nn.Embedding.
    
    PENTING: Jangan pakai fungsi ini untuk CopyNet target! 
    Untuk CopyNet, pakai tokens_to_indices_copynet yang mengembalikan extended indices.
    """
    unk_idx = word2idx.get(UNK_TOKEN, 3)  # fallback ke index 3 jika UNK_TOKEN tidak ada
    indices = []
    for t in tokens:
        if t in word2idx:
            indices.append(word2idx[t])
        else:
            indices.append(unk_idx)  # OOV → UNK, aman untuk embedding
    return indices


# Override fungsi tokenize asli agar protected_tokens menjadi opsional
_original_tokenize = tokenize

def tokenize(text: str, protected_tokens: List[str] = None) -> List[str]:
    """
    Memecah teks menjadi token. Jika protected_tokens tidak diberikan,
    gunakan SPECIAL_TOKENS sebagai default.
    """
    if protected_tokens is None:
        protected_tokens = SPECIAL_TOKENS
    return _original_tokenize(text, protected_tokens)